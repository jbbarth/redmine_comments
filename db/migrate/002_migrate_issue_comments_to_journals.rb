class MigrateIssueCommentsToJournals < ActiveRecord::Migration
  def self.up
    # Migrating issue comments to private notes in journals table
    comments = Comment.where(commented_type: Issue.class_name)
    ...
    puts "Destroy"
    Comment.where(commented_type: Issue.class_name).destroy_all
  end
